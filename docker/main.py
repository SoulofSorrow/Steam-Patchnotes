...
if page_url is None:
    print("Die Umgebungsvariable PAGE_URL ist nicht gesetzt.")
else:
    ...
    if response.status_code == 200:
        ...
        # Discord-Webhook-URL aus der Umgebungsvariable lesen
        discord_webhook_url = os.getenv("DISCORD_WEBHOOK_URL")

        if discord_webhook_url is None:
            print("Die Umgebungsvariable DISCORD_WEBHOOK_URL ist nicht gesetzt.")
        else:
            # Zeitintervall für sleep aus der Umgebungsvariable lesen
            sleep_interval = os.getenv("SLEEP_INTERVAL")
            if sleep_interval is not None:
                try:
                    sleep_interval = int(sleep_interval)
                except ValueError:
                    print("Ungültiger Wert für SLEEP_INTERVAL. Verwenden Sie eine ganze Zahl.")
                    sleep_interval = 30  # Fallback auf 300 Sekunden (5 Minuten)
            else:
                sleep_interval = 30  # Standardmäßig 300 Sekunden (5 Minuten)
            ...
            while True:
                ...
                for workshop_id in workshop_ids:
                    ...
                    if current_entry:
                        ...
                        if (last_saved_entry != current_entry) or (last_saved_title != current_title):
                            print(f"Es gibt eine Änderung in {url}:")
                            print(f'Titel: {current_title}')
                            print(f'Eintrag: {current_entry}')
                ...
                # Warten Sie für das angegebene Zeitintervall, bevor Sie erneut überprüfen
